...
@st.cache_data
def genera_dati_retail(n_campioni: int, livello_rumore: int, stato_casuale: int) -> tuple:
"""
Genera un dataset simulato di clienti retail con varie caratteristiche,
includendo archetipi di clienti e aggiungendo rumore.
"""
...
for i, (nome_archetipo, params) in enumerate(archetipi.items()):
    ...
    if maschera_rumore.any():
        # Applica un fattore di rumore solo ai punti selezionati
    ...
    for j in range(n):
        ...
        data.append([
            max(18, min(80, int(eta[j]))), # Età tra 18 e 80
            genere,
            max(20000, min(200000, int(reddito[j]))), # Reddito tra 20k e 200k
            max(0, int(visite_online_mensili[j])), # Visite online non negative
            max(10, float(dimensione_cestino_media[j])), # Dimensione cestino >= 10
            float(pct_acquisti_bio[j]),
            float(sensibilita_sconto[j]),
            max(0, int(visite_negozio_mensili[j])), # Visite negozio non negative
...
--- 4. Funzione per la Riduzione Dimensionalità ---
@st.cache_data
def riduci_dimensioni(X: np.ndarray, metodo: str, stato_casuale: int) -> tuple:
"""
...
if metodo == "PCA":
    ...
    # t-SNE richiede almeno 2 campioni e almeno 2 caratteristiche per funzionare
    if n_samples <= 1:
        st.warning("t-SNE richiede almeno 2 campioni. Impossibile applicare.")
        # Ritorna le prime 2 caratteristiche originali se disponibili
        return X[:, :min(n_features, 2)], "Caratteristiche Originali (Prime 2)"
    if n_features < 2:
        st.warning("t-SNE richiede almeno 2 caratteristiche per la visualizzazione 2D. Impossibile applicare.")
        # Ritorna le prime 2 caratteristiche originali se disponibili
        return X[:, :min(n_features, 2)], "Caratteristiche Originali (Prime 2)"

    # Perplexity deve essere tra 1 e n_samples-1
    perplexity_val = min(30, max(5, n_samples - 1))

    riduttore = TSNE(n_components=2, random_state=stato_casuale, perplexity=perplexity_val, n_jobs=-1)
    ridotto = riduttore.fit_transform(X)
...
--- 5. Funzione per Eseguire il Clustering ---
def esegui_clustering(X_scalato: np.ndarray, algoritmo: str, params: dict, stato_casuale: int) -> tuple:
"""
Esegue l'algoritmo di clustering selezionato.
Ritorna le etichette dei cluster, il modello (se applicabile), i centroidi (se K-Means) e l'inerzia (se K-Means).
"""
etichette = np.array([])
modello = None
centri = None
...
if algoritmo == "K-Means":
    ...
    if n_clusters_effettivo < 1:
        ...
        inertia = 0
    elif n_clusters_effettivo == 0:
        # Caso estremo con 0 campioni dopo i filtri
    ...
    if params['min_samples'] >= X_scalato.shape[0] or params['eps'] <= 0:
        ...
        centri = None # DBSCAN non ha centri espliciti o un modello unico

return etichette, modello, centri, inertia
--- 6. Funzione per Calcolare le Metriche di Clustering ---
def calcola_metriche(X: np.ndarray, etichette: np.ndarray) -> dict:
"""
...
# Per silhouette, Calinski-Harabasz e Davies-Bouldin, servono almeno 2 cluster e almeno 2 punti per cluster
if n_cluster_reali >= 2 and len(etichette_valide) >= 2:
    try:
...
return metriche
--- 7. Sidebar per i Controlli dell'Esperimento ---
with st.sidebar:
...
st.markdown("---")
# Pulsante per rieseguire l'analisi
if st.button("🔄 Esegui Analisi"):
    st.experimental_rerun()
--- 8. Esecuzione della Pipeline di Analisi ---
...
df_originale['etichetta_cluster'] = etichette_pred.astype(str) # Assicura che sia stringa per Plotly

metriche_risultati = calcola_metriche(X_scalato_ds, etichette_pred)
--- 9. Visualizzazione dei Cluster con Plotly ---
...
if X_ridotto_ds.shape[1] < 2:
    ...
    df_plot = pd.DataFrame({
        ...
        "Cluster": df_originale['etichetta_cluster'], # Usa la colonna già formattata
    ...
    if algoritmo == "K-Means" and centri_cluster_scalati is not None:
        ...
        if dim_reduction == "PCA":
            # Per proiettare i centroidi in PCA, dobbiamo ri-inizializzare e fittare un PCA
            # con gli stessi parametri usati per i dati, quindi trasformare i centroidi.
            temp_pca_model_for_centroids = PCA(n_components=min(X_scalato_ds.shape[1], 2), random_state=stato_casuale_ds)
            temp_pca_model_for_centroids.fit(X_scalato_ds) # Importante: fitta sui dati originali scalati
            if temp_pca_model_for_centroids.n_components_ >= 2:
                centri_per_plot = temp_pca_model_for_centroids.transform(centri_cluster_scalati)
        elif dim_reduction == "Caratteristiche Originali":
            # Se la visualizzazione è sulle caratteristiche originali, prendiamo le prime 2 dimensioni dei centroidi scalati
        ...
        if centri_per_plot is not None and centri_per_plot.shape[1] >= 2:
            fig.add_trace(go.Scatter(x=centri_per_plot[:, 0], y=centri_per_plot[:, 1],
                                     ...
                                     hoverinfo='none' # Non mostrare hover sui centroidi per evitare confusione
                                     ))
...
--- 10. Approfondimento sui Cluster ---
    ...
    if 'Genere' in df_originale.columns:
        if not df_clusterizzato_senza_rumore.empty:
            ...
            fig_genere = px.bar(genere_dist.reset_index().melt(id_vars=['etichetta_cluster']),
                                x='etichetta_cluster', y='value', color='variable', # 'variable' è il nome della colonna di genere dopo melt
                                title="Distribuzione Genere per Cluster", barmode='group',
                                labels={'value': 'Percentuale', 'variable': 'Genere'}, # Rinomina l'etichetta della legenda
    ...
    if 'Membro Carta Fedeltà' in df_originale.columns:
        if not df_clusterizzato_senza_rumore.empty:
            ...
            fig_carta_fedelta = px.bar(carta_fedelta_dist.reset_index().melt(id_vars=['etichetta_cluster']),
                                      x='etichetta_cluster', y='value', color='variable', # 'variable' è il nome della colonna booleana dopo melt
                                      title="Distribuzione Membro Carta Fedeltà per Cluster", barmode='group',
                                      labels={'value': 'Percentuale', 'variable': 'Membro Carta Fedeltà'}, # Rinomina l'etichetta della legenda
...
if not df_originale.empty:
    caratteristica_selezionata = st.selectbox("Seleziona una caratteristica da analizzare:", caratteristiche_numeriche, key="dist_char_select")
...
--- 11. Documentazione degli Algoritmi ---
with tab_docs:
...
---
### **K-Means**
- **Funzionamento**: Un algoritmo di partizionamento che mira a dividere $N$ osservazioni in $K$ cluster, dove ogni osservazione appartiene al cluster con il centroide (media dei punti del cluster) più vicino. Il processo è iterativo:
    1.  Inizializzazione di $K$ centroidi casuali.
    2.  Assegnazione di ogni punto al centroide più vicino.
    3.  Aggiornamento dei centroidi calcolando la media dei punti assegnati a ciascun cluster.
    4.  Ripetizione dei passi 2 e 3 fino a convergenza (i centroidi non cambiano più in modo significativo).
- **Punti Chiave**:
    ...
    - Sensibile alla **posizione iniziale dei centroidi** (per questo l'opzione `n_init='auto'` esegue l'algoritmo più volte con diverse inizializzazioni e sceglie quella con la minore inerzia).
    - Sensibile agli **outlier**, che possono attrarre i centroidi e distorcere i cluster.
    - È generalmente **veloce** e scalabile per grandi dataset con un $K$ ragionevole.

---
### **DBSCAN (Density-Based Spatial Clustering of Applications with Noise)**
- **Funzionamento**: Un algoritmo di clustering basato sulla densità che raggruppa i punti che sono vicini tra loro e identifica i punti isolati come rumore. Non richiede di specificare il numero di cluster in anticipo.
- **Concetti Chiave**:
    - **Epsilon ($\epsilon$)**: Il raggio massimo per considerare due campioni come vicini.
    - **Min Samples (MinPts)**: Il numero minimo di campioni (incluso il punto stesso) in un raggio $\epsilon$ affinché un punto sia considerato un "core point".
- **Tipi di Punti**:
...
- **Punti Chiave**:
    ...
    - È **robusto agli outlier**, identificandoli esplicitamente come punti di rumore.
    - La sua performance dipende molto dalla scelta dei parametri `eps` e `MinPts`.
    - Può faticare con cluster di **densità molto diverse**, poiché i parametri sono globali.
""")
--- 12. Footer dell'Applicazione ---
st.markdown("---")
st.markdown("""
